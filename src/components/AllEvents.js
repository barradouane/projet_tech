...
export default function AllEvents({ siteProp }) {
  ...
  const fetchEvents = async () => {
    try {
      ...
      let url = "http://localhost/projet_tech-WorkingVersion/src/backend/get_data.php"
      if (site) {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {eventItems.map((event, index) => (
              <EventsData
                id={event.id}
                key={index}
                image={event.image ? `http://localhost/projet_tech-WorkingVersion/src/backend/uploads/${event.image}` : "https://via.placeholder.com/300"}
                heading={event.titre}
