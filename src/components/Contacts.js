...
export default function Contacts() {
    ...
    const fetchContactsService = async (service) => {
        try {
            const response = await fetch(
                `http://localhost/projet_tech-latest/src/backend/get_contacts.php?service=${service}`
            );
    ...
    const fetchContacts = async (type, value) => {
        try {
            const response = await fetch(
                `http://localhost/projet_tech-latest/src/backend/get_contacts.php?${type}=${value}`
            );
            const data = await response.json();
