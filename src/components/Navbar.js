...
import { FaSignOutAlt, FaBars, FaTimes, FaHome, FaBriefcase, FaAddressBook, FaLink } from "react-icons/fa"
...
export default function Navbar({ city }) {
  ...
  const quickLinksMenu = showQuickLinks && (
    <ul className="rounded-[10px] absolute left-0 sm:left-auto sm:right-0 top-full bg-secondary text-light mt-0 w-full sm:w-[200px] sm:max-h-[450px] max-h-[300px] overflow-y-auto z-20">
  ...
  return (
    <nav className="flex justify-between items-center px-[30px] py-0 shadow-[0_5px_15px_rgba(0,0,0,0.25)] w-[98%] h-[80px] rounded-[13px] fixed top-[20px] left-1/2 transform -translate-x-1/2 z-[9999] bg-transparent text-white backdrop-blur-[30px] border-[3px] border-white/20 p-[30px] ">
      <img
      ...
      <ul
        className={`sm:mt-0 mt-2 sm:flex sm:items-center sm:static sm:w-auto sm:opacity-100 sm:flex-row sm:gap-2 sm:ml-4
      ...
      >
        ...
        <li
          className="w-full sm:w-auto text-secondary text-xl py-2 sm:py-0 hover:bg-secondary hover:text-light hover:rounded-[10px] transition-all duration-300 ease-in-out cursor-pointer ml-auto"
